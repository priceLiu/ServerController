...
namespace SYSInfo
{
    ...
    public partial class Form1 : Form
    {
        ...
        public bool bColorGlobal,bTransparency,bAero,bAeroFrame,bGradient,bTexture,bTextureBlue,bTextureGrey,bHDDPBar = false,bCPU = false,bCPUText = false;
        public bool bNetTP = false, bNetIP = false, bNetMask = false, bNetMac = false, bNetGW = false,bFixed = false;
 ...
 //               this._hddBar[i].MouseDown += new System.Windows.Forms.MouseEventHandler(this.PBAR_MOUSE_DOWN);
        ...
        public void _update_visuals()
        {
            ...
            for (int i = 0; i < 20; i++)
            {
                if (iFunction[i] > -1)
                {
                    ...
                    if ((i != 3||bHDDPBar == false))  //exclude HDD ProgressBar
                    {
            ...
            int iIndex = 0;
            if (sText == "" && iFunction[4] == -1 && iFunction[3] == -1 && iFunction[10] == -1 && iFunction[11] == -1 && iFunction[12] == -1)
                settingsDialog();
            else
            {
                if (panel2.Visible && bCPU)
                {
                    if (bCPUText)
                    {
            ...
            //textureBlueToolStripMenuItem.Checked = bTextureBlue;
            //textureGreyToolStripMenuItem.Checked = bTextureGrey;
            transparentToolStripMenuItem.Checked = bTransparency;
            aeroEffektabVistaToolStripMenuItem.Checked = bAero;
            textureToolStripMenuItem.Checked = bTexture;//(bTextureBlue || bTextureGrey) ? true : false;
        ...
        public string top5ProcessCPUUsage()
        {
            ...
            if (plist != null)
            {
                ...
                {   
                    ...
                    iLength = sItemName.Length > 14 ? 14 : sItemName.Length;
        ...
        {
            if (e.Button == MouseButtons.Left && !bFixed)
            {
